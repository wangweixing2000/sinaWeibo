...
def get_qrcode():
    ...
    """
    qrcode_before = "http://login.sina.com.cn/sso/qrcode/image?entry=weibo&size=180&callback=STK_" + str(int(time.time() * 10000))
    ...
    qrcode_qrid = re.search(r'"qrid":"(?P<qrid>[\w\-]*)"', qrcode_before_data)
    if qrcode_qrid:
        qrcode_qrid = qrcode_qrid.group("grid")
